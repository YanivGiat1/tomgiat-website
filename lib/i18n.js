...
// A cell containing exactly "-" is a deliberate "make this blank" marker
// (a plain empty cell can't mean that, since blank already means "keep
// the default" — this gives editors a way to explicitly clear optional
// text like a section heading).
  ...
  useEffect(() => {
    ...
    fetch(TRANSLATIONS_CSV_URL)
      ...
      .then((csvText) => {
        ...
        // press/facts are ordered, structured content curated in code (URLs,
        // grouping flags, etc.) — matching sheet rows to array items by
        // position breaks the moment that array is reordered or resized, so
        // those two are intentionally excluded from the sheet overlay.
        const isStructuredArrayKey = (key) =>
          key.startsWith('oneEyeOpen.press.') || key.startsWith('oneEyeOpen.facts.');

        const heFlat = {};
        const enFlat = {};
        data.forEach((row) => {
          const key = (row.key || '').trim();
          if (!key || isStructuredArrayKey(key)) return;
